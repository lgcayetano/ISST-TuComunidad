...
public interface UsuarioRepository extends CrudRepository<Usuario, Integer> {
    ...
    @Query(value = "SELECT usuario.id AS id, usuario.nombre AS nombre, usuario.email AS email, usuario.estado AS estado FROM Usuario WHERE idcomunidad=?1 ORDER BY id DESC", nativeQuery = true)
